...
function createTestScenario(
  ...
  const goal: Goal = {
    ...
    horizonYears: 5,
    tiers: {
      basic: { targetAmount: [Math.round(targetAmount * 0.9), targetAmount], priority: 1 },
      ambitious: { targetAmount: [Math.round(targetAmount * 0.95), targetAmount], priority: 2 },
    },
  };
...
describe('Exact 90% Confidence Test with Different Goal Amounts', () => {
  it('should return 90% confidence for Method 1 and Method 2 when networth exactly meets goal with ₹10L target, 1000 SIP, 10% stretch, and 10% stepup', () => {
    const scenario = createTestScenario(
      'test-goal-90-amount-10l',
      'Test Goal 90% Confidence with ₹10L Target',
      1000000, // ₹10L
      1000,
      10,
      10
  ...
  it('should return 90% confidence for Method 1 and Method 2 when networth exactly meets goal with ₹1Cr target, 1000 SIP, 10% stretch, and 10% stepup', () => {
    const scenario = createTestScenario(
      'test-goal-90-amount-1cr',
      'Test Goal 90% Confidence with ₹1Cr Target',
      10000000, // ₹1Cr
      1000,
      10,
      10
