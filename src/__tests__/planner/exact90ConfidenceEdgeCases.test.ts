...
import { corpusAtTime, corpusAtTimeWithStepUp, annualToMonthlyReturn } from '../../utils/math';

function calculateRequiredCorpusFor90Confidence(
  targetAmount: number,
  horizonYears: number,
  ...
  let sipFV: number;
  if (stepUpPercent > 0) {
    sipFV = corpusAtTimeWithStepUp(0, initialAvailableSIP, stepUpPercent, lowerMonthlyReturn, months);
  } else {
    sipFV = initialAvailableSIP === 0 ? 0 : initialAvailableSIP * ((Math.pow(1 + lowerMonthlyReturn, months) - 1) / (lowerMonthlyReturn || 1));
  }
...
function createTestScenario(
  ...
  const requiredCorpus = calculateRequiredCorpusFor90Confidence(
    targetAmount,
    goal.horizonYears,
    optimalAllocation,
    assetClassDataMap,
...
function runTest(goal: Goal, assetClasses: AssetClasses, customerProfile: CustomerProfile, sipInput: SIPInput, optimalAllocation: AssetAllocation[], requiredCorpus: number) {
  ...
  // Use wider range for edge cases with high percentages
  let lowCorpus = requiredCorpus * 0.5;
  let highCorpus = requiredCorpus * 2.0;
...
describe('Exact 90% Confidence Test - Edge Cases', () => {
  it('should return 90% confidence for Method 1 and Method 2 when networth exactly meets goal with 0 SIP, 10% stretch, and 0% stepup', () => {
    const scenario = createTestScenario(
      'test-goal-90-edge-0-sip-stretch',
      'Test Goal 90% Confidence with 0 SIP and 10% Stretch',
      0,
      10,
      0
    );
  ...
  it('should return 90% confidence for Method 1 and Method 2 when networth exactly meets goal with 1000 SIP, 0% stretch, and 0% stepup', () => {
    const scenario = createTestScenario(
      'test-goal-90-edge-base-sip',
      'Test Goal 90% Confidence with Base SIP Only',
      1000,
      0,
      0
    );
  ...
  it('should return 90% confidence for Method 1 and Method 2 when networth exactly meets goal with 100 SIP, 50% stretch, and 25% stepup', () => {
    const scenario = createTestScenario(
      'test-goal-90-edge-high-percentages',
      'Test Goal 90% Confidence with High Percentages',
      100,
      50,
      25
    );
