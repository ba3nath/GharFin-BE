...
/**
 * Calculate required initial corpus to meet target at exactly 90% confidence with step-up SIP
 * This works backwards from the target to find the corpus needed
 */
function calculateRequiredCorpusFor90ConfidenceWithStepUp(
  targetAmount: number,
  horizonYears: number,
  ...
  // Work backwards: targetAmount = corpusAtTimeWithStepUp(initialCorpus, initialMonthlySIP, stepUpPercent, lowerMonthlyReturn, months)
  const sipFV = corpusAtTimeWithStepUp(0, initialMonthlySIP, stepUpPercent, lowerMonthlyReturn, months);
...
function createTestScenario(
  ...
  const requiredCorpus = calculateRequiredCorpusFor90ConfidenceWithStepUp(
    targetAmount,
    goal.horizonYears,
    optimalAllocation,
    assetClassDataMap,
...
describe('Exact 90% Confidence Test with High Step-Up', () => {
  it('should return 90% confidence for Method 1 and Method 2 when networth exactly meets goal with 1000 SIP and 20% stepup', () => {
    const { goal, assetClasses, customerProfile, sipInput, optimalAllocation, requiredCorpus } = createTestScenario(
      'test-goal-90-high-stepup-20',
      'Test Goal 90% Confidence with 20% Step-up',
      1000,
      20
    );
    ...
    // Use wider range for high step-up percentages
  ...
  it('should return 90% confidence for Method 1 and Method 2 when networth exactly meets goal with 1000 SIP and 30% stepup', () => {
    const { goal, assetClasses, customerProfile, sipInput, optimalAllocation, requiredCorpus } = createTestScenario(
      'test-goal-90-high-stepup-30',
      'Test Goal 90% Confidence with 30% Step-up',
      1000,
      30
    );
