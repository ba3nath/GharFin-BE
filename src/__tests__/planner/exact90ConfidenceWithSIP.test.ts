...
describe('Exact 90% Confidence Test with SIP and Step-up', () => {
  it('should return 90% confidence for Method 1 and Method 2 when networth exactly meets goal with 1000 SIP and 10% stepup', () => {
    ...
    while (iterations < maxIterations && Math.abs(method1Confidence - 90) > 1) {
      ...
      // Use helper to create test profile
    ...
    // Use the found corpus - create final profile using helper
    ...
    // This verifies the envelope method correctly calculates 90% confidence with step-up SIP
    expect(method1Row?.confidencePercent).toBeGreaterThanOrEqual(85);
    expect(method1Row?.confidencePercent).toBeLessThanOrEqual(95);
    ...
    // Final assertions:
    // 1. Method 1 correctly achieves ~90% confidence with appropriate corpus and step-up SIP
