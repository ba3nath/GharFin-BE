...
/**
 * Calculate required initial corpus to meet target at exactly 90% confidence with stretch SIP
 * This works backwards from the target to find the corpus needed
 * Note: Stretch SIP increases available SIP, so we use the stretched amount
 */
function calculateRequiredCorpusFor90ConfidenceWithStretch(
  targetAmount: number,
  horizonYears: number,
  ...
  // Calculate available SIP with stretch
  const stretchSIP = monthlySIP * (1 + stretchSIPPercent / 100);
  const availableSIP = Math.max(monthlySIP, stretchSIP);
  ...
  // Work backwards: targetAmount = corpusAtTime(initialCorpus, availableSIP, lowerMonthlyReturn, months)
  // corpusAtTime = initialCorpus * (1 + r)^months + SIP * ((1 + r)^months - 1) / r
  // Solving for initialCorpus:
  // initialCorpus = (targetAmount - SIP * ((1 + r)^months - 1) / r) / (1 + r)^months

  const sipFV = availableSIP === 0 ? 0 : availableSIP * ((Math.pow(1 + lowerMonthlyReturn, months) - 1) / (lowerMonthlyReturn || 1));
...
describe('Exact 90% Confidence Test with Stretch SIP', () => {
  it('should return 90% confidence for Method 1 and Method 2 when networth exactly meets goal with 1000 SIP and 10% stretch', () => {
    ...
    // Setup: 1000 SIP, 10% stretch, 0% stepup
    ...
    // This verifies the envelope method correctly calculates 90% confidence with stretch SIP
    expect(method1Row?.confidencePercent).toBeGreaterThanOrEqual(85);
    expect(method1Row?.confidencePercent).toBeLessThanOrEqual(95);
    ...
    // Final assertions:
    // 1. Method 1 correctly achieves ~90% confidence with appropriate corpus and stretch SIP
